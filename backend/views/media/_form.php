...
<div class="media-form">
    ...
    <div class="form-group">
        ...
        <?php elseif (preg_match('/^video\//', $model->mime_type)): ?>
            <video controls class="full-width">
                ...
                <?= Yii::t('cms', 'Your browser does not support HTML5 video.') ?>
            </video>
        <?php elseif (preg_match('/^audio\//', $model->mime_type)): ?>
            <audio controls class="full-width">
                <source src="<?= $model->getUploadUrl() . ArrayHelper::getValue($metadata, 'versions.full.url') ?>"
                        type="<?= $model->mime_type ?>">
                <?= Yii::t('cms', 'Your browser does not support HTML5 audio.') ?>
            </audio>
        <?php else: ?>
            <?= Html::textInput(
    ...
    <?= $form->field($model, 'comment_status')->checkbox([
        'label' => Yii::t('cms', 'Allow comments on this media'),
        'checked' => true,
        'value' => 'open',
        'uncheck' => 'close',
    ]) ?>
    ...
    <div class="form-group">
        <?= Html::submitButton(Yii::t('cms', 'Update'), ['class' => 'btn btn-flat btn-primary']) ?>
    </div>
</div>
