...
class ThemeController extends Controller
{
    ...
    public function behaviors()
    {
        return [
            'access' => [
                ...
                'rules' => [
                    [
                        'actions' => ['index', 'upload', 'detail', 'install', 'delete', 'ajax-detail', 'customize'],
                        'allow'   => true,
                        'roles'   => ['administrator'],
                    ],
                ],
        ...
        if (($model->file = UploadedFile::getInstance($model, 'file')) && $model->validate()) {
            ...
            if (!$model->file->saveAs($themeTempPath)) {
                return $this->render('upload', [
                    'model' => $model,
                    'errors' => [Yii::t('cms', 'Failed to move uploaded file')],
                ]);
            }
            ...
            if (!$zipArchive->extractTo($this->_tmp)) {
                ...
                return $this->render('upload', [
                    'model' => $model,
                    'errors' => [Yii::t('cms', 'Failed to extract file.')],
                ]);
            }
            ...
            if (is_dir($this->_dir . $baseDir)) {
                FileHelper::removeDirectory($this->_tmp);
                $errors[] = Yii::t('cms', 'Theme with the same directory already exist.');
            } else {
                ...
                Yii::$app->getSession()->setFlash('success', Yii::t('cms', 'Theme successfully uploaded'));
    ...
    {
        $model = Option::find()->where(['group' => 'theme'])->indexBy('name')->all();
        ...
        if ($options = Yii::$app->request->post('Option')) {
            ...
            foreach ($options as $optionName => $option) {
                if (in_array($optionName, $imageOptions)) {
                    $file = UploadedFile::getInstanceByName($optionName . '_file');
                    if ($file && !$file->hasError) {
                        $path = Yii::getAlias('@public/uploads/theme/' . $model[$optionName]->value);
                        if (is_file($path)) {
                            unlink($path);
                        }
                        $newName = md5(time()) . '-' . $file->name;
                        $file->saveAs(Yii::getAlias('@public/uploads/theme/' . $newName));
    ...
    public function actionInstall($theme)
    {
        ...
        if (Option::set('theme', $theme)) {
            Yii::$app->getSession()->setFlash('success', Yii::t('cms', "Theme successfully installed."));
        }
